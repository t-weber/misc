	...
	m_vecCamX{m::create<t_vec_gl>({1.,0.,0.,0.})}, m_vecCamY{m::create<t_vec_gl>({0.,1.,0.,0.})}
{
	...
	{
		this->tick(std::chrono::milliseconds(1000 / 60));
	});
...
void GlPlot::SetObjectPortal(std::size_t idx, bool portal)
{
	if(idx >= m_objs.size()) return;
	m_objs[idx].m_portal = portal;
}


void GlPlot::SetObjectPortalMatrix(std::size_t idx, const t_mat_gl& mat)
{
	if(idx >= m_objs.size()) return;
	m_objs[idx].m_mat_portal = mat;
}


std::size_t GlPlot::AddPlane(t_real_gl size,
	t_real_gl x, t_real_gl y, t_real_gl z,
	t_real_gl nx, t_real_gl ny, t_real_gl nz,
	t_real_gl r, t_real_gl g, t_real_gl b, t_real_gl a)
{
	auto solid = m::create_plane<t_mat_gl, t_vec3_gl>(
		m::create<t_vec3_gl>({nx,ny,nz}), size);
	auto [triagverts, norms, uvs] = 
		m::subdivide_triangles<t_vec3_gl>(
			m::create_triangles<t_vec3_gl>(solid), 2);

	auto obj = CreateTriangleObject(
		std::get<0>(solid), triagverts, norms, 
		m::create<t_vec_gl>({r,g,b,a}), true);
...
// if the frame is not already updated by the timer, directly update it
		...
		t_mat_gl matPortal = m::unit<t_mat_gl>();	// current portal

		// TODO: use a unique identifier for each portal
		GLint portal_stencil_val = 123;
		GLint stencil_mask = 0xff;

		// pass 0: draw portals into stencil buffer
		// pass 1: draw scene through portals
		// pass 2: draw scene without portals
		for(int pass=0; pass<3; ++pass)
		{
			...
			// draw portals into stencil buffer
			if(pass == 0)
			{
				pGl->glEnable(GL_STENCIL_TEST);
				pGl->glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);

				// only write to stencil buffer if both the stencil
				// and the depth test pass
				pGl->glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);
			}

			// draw scene through portals
			else if(pass == 1)
			{
				// only read from the stencil buffer now
				pGl->glEnable(GL_STENCIL_TEST);
				pGl->glStencilMask(0);
				pGl->glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);

				pGl->glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
				// TODO: test against individual portal value in stencil buffer
				pGl->glStencilFunc(GL_EQUAL, portal_stencil_val, stencil_mask);
			...
			for(auto& obj : m_objs)
			{
				...
				if(pass == 0)
				{
					// only draw portals into the stencil buffer
					if(obj.m_portal)
					{
						pGl->glStencilMask(stencil_mask);

						// TODO: write different stencil values for different portals
						pGl->glStencilFunc(GL_ALWAYS, portal_stencil_val, stencil_mask);

						// remember current portal matrix for subsequent rendering passes
						matPortal = obj.m_mat_portal;
					}
					else
					{
				...
				m_pShaders->setUniformValue(m_uniMatrixObj,
					pass==1 ? matPortal * obj.m_mat : obj.m_mat);
...
// ----------------------------------------------------------------------------
void GlPlot::tick([[maybe_unused]] const std::chrono::milliseconds& ms)
{
	// TODO
	UpdateCam();
}
...
{
	[[maybe_unused]] bool bLeftDown = 0, bRightDown = 0, bMidDown = 0;
...
{
	[[maybe_unused]] bool bLeftUp = 0, bRightUp = 0, bMidUp = 0;
